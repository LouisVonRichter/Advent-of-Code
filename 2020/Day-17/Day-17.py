...
def part1(data):
    ...
    grid = [
        [["." for _ in range(columns + expanded)] for _ in range(rows + expanded)]
        ...
        for i in range(1 + range_expanded):
            for j in range(rows + range_expanded):
                for k in range(columns + range_expanded):
                    ...
                    if (grid[i + si][j + sj][k + sk] == ACTIVE) and (
                        not (2 <= count_active <= 3)
                    ):
                        grid_clone[i + si][j + sj][k + sk] = INACTIVE

                    if (grid[i + si][j + sj][k + sk] == INACTIVE) and (
                        count_active == 3
                    ):
